"""
memoscape/memory_engine.py
MemoryEngine — pluggable on_loading callback for MemoscapeApp.

Responsibilities
----------------
- Receive a MemoscapeApp reference when the FSM enters LOADING.
...
Usage
-----
    from memoscape.memory_engine import MemoryEngine, EngineConfig

    engine = MemoryEngine(config=EngineConfig())
    app = MemoscapeApp(config=AppConfig(), on_loading=engine)
    asyncio.run(app.run())
...
from memoscape.fsm import MemoryCard
...
class MemoryEngine:
    """
    Async callable that satisfies the MemoscapeApp.on_loading signature::

        async def __call__(self, app: MemoscapeApp) -> None

    Instantiate once and pass as on_loading::

        engine = MemoryEngine(provider=my_provider)
        app = MemoscapeApp(on_loading=engine)
    """
    ...
    async def __call__(self, app: Any) -> None:  # app: MemoscapeApp
        self._call_count += 1
        t0 = time.monotonic()
