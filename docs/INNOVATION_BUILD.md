...
| B10 | Figment grammar: gesture/place/presence/cadence/ledger/slot events (5.1) | partial | `reality_compiler/v2/figment.py` grammar + `budgets.py` | ✅ 5.1 #1 gestures (`imu:<g>`), #2 place, #3 presence (all enforced at verify), #5 ledger emits (`record: true` → interpreter tracks + `log_recorded()` drains to the Vault performance log). Follow-on: host-firing of place/bond + #4 cadence / #6 slot |
