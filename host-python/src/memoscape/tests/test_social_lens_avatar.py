...
from memoscape.face_recall.renderer import (
    AVATAR_SIZE, AvatarCache, build_person_context_card, why_this_person,
)
from memoscape.face_recall.schema import ContactRecord, FaceRecallResult, MatchResult
...
def test_no_face_never_produces_card():
    result = FaceRecallResult(match=None, frame_confidence=0.2, no_face=True)
...
def test_no_match_never_produces_card():
    ...
    result = FaceRecallResult(match=None, frame_confidence=0.9, no_match=True)
    assert build_person_context_card(result, avatar_cache=AvatarCache()) is None


def test_below_threshold_match_never_produces_card():
    result = FaceRecallResult(
...
def test_avatar_cache_untouched_by_non_contacts():
    ...
    build_person_context_card(
        FaceRecallResult(match=None, frame_confidence=0.9, no_match=True),
        avatar_cache=cache,
    )
    assert len(cache) == 0
