"""Tests for SocialLens schema dataclasses."""
import pytest
from dreamlayer.social_lens.schema import (
    ContactRecord, MatchResult, SocialLensResult,
)
...
class TestSocialLensResultNoFace:
    def test_no_face_card_type(self):
        r = SocialLensResult(match=None, frame_confidence=0.0, no_face=True)
        card = r.to_hud_card()
...
class TestSocialLensResultNoMatch:
    def test_no_match_card(self):
        r = SocialLensResult(match=None, frame_confidence=0.8, no_match=True)
        card = r.to_hud_card()
