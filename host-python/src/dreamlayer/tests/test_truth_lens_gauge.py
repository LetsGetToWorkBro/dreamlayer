...
from dreamlayer.truth_lens.renderer import TruthLensRenderer
from dreamlayer.truth_lens.schema import (
    AUFrame, CredibilityVector, TruthLensResult, LinguisticFrame, ProsodyFrame,
)
...
def make_result(deception=0.7, confidence=0.8, au_z=2.0, voice_z=0.5,
    ...
    return TruthLensResult(credibility=cv, contact_name="Jordan",
...
def test_gauge_has_exactly_nine_stages():
    ...
    assert len(stages) == 9
    assert [s["name"] for s in stages] == list(TruthLensResult.GAUGE_STAGES)
...
# ---------------------------------------------------------------------------
# TruthLensRenderer routing + suppression
# ---------------------------------------------------------------------------

def test_renderer_emits_gauge_card():
    card = TruthLensRenderer().render(make_result())
...
def test_renderer_suppresses_low_deception():
    assert TruthLensRenderer().render(make_result(deception=0.1)) is None


def test_renderer_suppresses_low_confidence():
    assert TruthLensRenderer().render(
        make_result(confidence=0.1, stranger=False)) is None


def test_renderer_none_passthrough():
    assert TruthLensRenderer().render(None) is None
