...
on the command line when the `network` capability was withheld. That is a lint
over a list of strings. It cannot tell whether the kernel honoured the flag, so
the whole tier could be inert without one assertion there moving — the same
...
one catches what. `argv[0] == "bwrap"` and the `--unshare-net` check catch *the
flag was dropped* — the same direction `test_os_sandbox.py` already covers. Only
the protocol-line and errno checks catch *the flag is present and the boundary
is inert*, which is the direction a construction test cannot see and the reason
this file exists.
"""
...
# The untrusted plugin. `register()` runs inside the jail, so the reach happens
# in there and its outcome leaves over the child's own protocol on stdout — the
# only channel out. Not because the jail is read-only: bwrap's new root is a
# writable tmpfs, and measured through this very wrapper, `/` and `/tmp` accept
# writes while the bound host paths give EROFS. It is the only channel out
...
def _bwrap_or_say_why():
    ...
    CI leg installs, and the flag this file asserts on (`--unshare-net`) is the
    bwrap branch of `wrapper()`. The nsjail branch builds a different command
...
def _write_pkg(root):
    ...
    pkg = root / "netprobe"
    pkg.mkdir()
    (pkg / "plugin.py").write_text(PROBE_SRC)
    (pkg / "manifest.json").write_text(json.dumps(
        {"name": "netprobe", "version": "0.1.0", "entry": "plugin:make",
         "api": "2", "requires": []}))
    return pkg


def _run_jailed(pkg, caps):
    ...
    proc = subprocess.run(argv, input='{"op":"init"}\n', capture_output=True,
