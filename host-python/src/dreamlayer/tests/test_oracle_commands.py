"""test_oracle_commands.py — "Hey Oracle, *do* something" + Oracle's voice.

Oracle runs the device on command, answers questions from your brain, and
...
def test_parses_device_commands():
    assert parse_command("Hey Oracle, turn on focus").kind == "focus"
...
# -- Oracle runs the device ---------------------------------------------------

def test_oracle_executes_a_local_switch_and_confirms():
    br = FakeBridge()
    orc = Orchestrator(br)
    out = orc.ask_oracle("turn on focus")
...
def test_oracle_toggles_incognito_and_rewind():
    orc = Orchestrator(FakeBridge())
    assert orc.ask_oracle("go incognito")["executed"] and orc.incognito
    assert orc.ask_oracle("rewind my day")["intent"] == "rewind"


def test_cross_device_command_comes_back_as_an_intent():
    orc = Orchestrator(FakeBridge())
    out = orc.ask_oracle("sync my contacts")
    assert out["intent"] == "sync" and out["executed"] is False   # app relays to the Brain
    assert "contacts" in out["text"]


# -- Oracle answers, in voice -------------------------------------------------

def test_a_question_gets_an_answer_card_in_persona():
    br = FakeBridge()
    orc = Orchestrator(br)
    out = orc.ask_oracle("what did Marcus need?")
    assert out["intent"] == "recall"
    assert _replies(br)[-1]["type"] == "OracleReplyCard"          # answered as text


def test_hey_oracle_wakes_then_runs_the_command():
    br = FakeBridge()
    orc = Orchestrator(br)
    out = orc.hear("Hey Oracle, turn on focus")
    assert out["intent"] == "focus" and orc.focus_active()
    kinds = [c["type"] for c in br.raw if c.get("t") == "card"]
    assert "ListeningCard" in kinds and "OracleReplyCard" in kinds
...
def test_persona_lines_are_in_character():
    ...
    assert "Oracle" in persona.PERSONA_PROMPT
