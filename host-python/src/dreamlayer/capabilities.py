...
CAPABILITIES: Tuple[Cap, ...] = (
    ...
    Cap("typed_pipeline", "Traced RC stage pipeline", "structured",
        ...
        gain="the adapter already records what ran and where it failed, with no dependency at all — installing this buys the TYPED agent surface (validated stage IO, retries on a schema miss), which nothing in the tree asks for yet", impact=1, before=4, after=4),
    ...
    Cap("persona_tuning", "Human-in-the-loop persona classifier", "intelligence",
        ...
        gain="the classifier already applies any rule callable you hand it — installing this is what lets you BUILD that rule by example (a FunctionClassifier drawn from labelled cases) instead of writing it by hand; nothing in the tree builds one yet", impact=1, before=3, after=3),
